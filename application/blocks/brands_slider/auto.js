Concrete.event.bind('btBrandsSlider.rowOne.edit.open', function (options, settings) {
    ...
    var ftImageImageOne = $(newField).find('.ft-image-rowOne-imageOne-file-selector');
            if ($(ftImageImageOne).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageImageOne).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorImageOne = $(newField).find('.ft-image-rowOne-imageOne-page-selector');
            if ($(pageSelectorImageOne).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorImageOne).attr('id'),
                        components: config.components
                    });
                });
            }
...
});Concrete.event.bind('btBrandsSlider.rowTwo.edit.open', function (options, settings) {
    var container = $('#' + settings.id);
    var hbTemplate = Handlebars.compile($(container).find('.repeatableTemplate').html());
    var sortableItems = $(container).find('.sortable-items');
    var slideSpeed = 200;
    ...
    var updateItem = function (newField) {
        $(newField).find(".launch-tooltip").tooltip({container: "#ccm-tooltip-holder"});
    var ftImageImageTwo = $(newField).find('.ft-image-rowTwo-imageTwo-file-selector');
            if ($(ftImageImageTwo).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageImageTwo).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorImageTwo = $(newField).find('.ft-image-rowTwo-imageTwo-page-selector');
            if ($(pageSelectorImageTwo).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorImageTwo).attr('id'),
                        components: config.components
                    });
                });
            }
