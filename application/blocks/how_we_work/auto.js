Concrete.event.bind('btHowWeWork.features.edit.open', function (options, settings) {
    ...
    blockDesignerEditor('#' + ($(newField).find('textarea.ft-features-featureDescription').attr('id')).replace(/(:|\.|\[|\]|,|=|@)/g, "\\$1"));
    };
        ...
        if (deleteIt === true) {
            ...
            $(sortableItem).slideUp(slideSpeed, function () {
                if(typeof CKEDITOR !== 'undefined'){
                    var blockDesignerEditorInstance = CKEDITOR.instances[$(sortableItem).find('textarea.ft-features-featureDescription').attr('id')];
